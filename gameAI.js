...
*/
    player = game.add.sprite(100,700,'paddle');
    game.physics.arcade.enable(player);
    // player.body.bounce.set(0.8);
    //player.body.immovable = true;
    player.body.collideWorldBounds = true;
    player.body.immovable = true;
    tween = game.add.tween(player).to({ x: 800 }, 3000, Phaser.Easing.Linear.None, true, 0, 1000, true)
  //  tween.from({ x: 100 }, 3000, Phaser.Easing.Linear.None, true, 0, 1000, true)
...
// time event
game.time.events.add(Phaser.Timer.SECOND * 4, this.shoot, this);
},
...
if (ball.y < 700 && ball.x < 200) {

  game.physics.arcade.moveToXY(player, 0, player.y, 800, 300);
}
else if (ball.y < 700 && ball.x < 400 && ball.x > 200 ){

  game.physics.arcade.moveToXY(player,300 ,player.y , 800, 300);
}
else if (ball.y < 700 && ball.x > 400 && ball.x < 600){
  game.physics.arcade.moveToXY(player, 500 , player.y, 800, 300);

} else if (ball.y < 700 && ball.x > 600 && ball.x < 800){
game.physics.arcade.moveToXY(player, 700 ,player.y, 800, 300);

} else if (ball.y < 700 && ball.x > 800 && ball.x < 1000 ){
  game.physics.arcade.moveToXY(player, 1000, player.y, 800, 300);

} else if (ball.y < 700 && ball.x > 1000) {
      game.physics.arcade.moveToXY(player, 1200, player.y, 800, 300);

} /*else if (ball.y < 700 && ball.x > 0){
  game.physics.arcade.moveToXY(player, 600, player.y, 800, 300);
}
*/
